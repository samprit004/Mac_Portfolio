...
/* ── Data ── */
const DETAIL_ROWS = [
  { label: 'mobile',   value: '+91 62911 20228',          href: 'tel:+916291120228' },
...
/* ── Social button (identical to desktop, shrunk for mobile) ── */
const SocialButton = ({ href, icon, label, className, onClick, copied = false, active = false }) => {
  const iconEl = (
    <span
      className={`flex size-9 items-center justify-center rounded-full border border-black/5 shadow-sm transition duration-150 ${className}`}
      style={active ? { outline: '2px solid rgba(52,120,246,0.25)', outlineOffset: '2px' } : undefined}
    >
      {icon}
    </span>
  )
...
/* ── Mobile Contact — same UI as desktop, no window chrome ── */
const MobileContact = () => {
  ...
  return (
    <div
      className="flex h-full min-h-0 flex-col overflow-y-auto overflow-x-hidden font-['SF_Pro_Text','SF_Pro_Display','-apple-system','BlinkMacSystemFont','Segoe_UI',sans-serif]"
    ...
    >
      ...
      <div className="mx-auto flex w-full max-w-[520px] shrink-0 items-center justify-center gap-5 px-[18px] pt-7 text-left sm:max-w-[620px] sm:px-5">
        ...
        <div className="flex min-w-0 flex-col justify-center">
          ...
          <p className="mt-1 text-[13px] font-normal" style={{ color: 'var(--contact-subtitle)' }}>Full Stack Developer · React · Node.js · MongoDB</p>
          <p className="mt-1 flex items-center gap-1 text-[12px]" style={{ color: 'var(--window-muted)' }}>
            <PinIcon />Kolkata, India
          </p>
        </div>
      </div>
